...
class _DefaultBannerWidgetState<T> extends State<DefaultBannerWidget<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isLoading || widget.images.isEmpty) {
      return ShimmerContainerWidget(height: 180.h);
    }
    ...
    if (widget.images.length == 1) {
      return Padding(
        ...
        child: _buildImage(widget.images.first, 0),
      );
    }
      ...
      children: [
        carousel.CarouselSlider(
          items: widget.images
              .asMap()
              .entries
              .map((entry) => _buildImage(entry.value, entry.key))
              .toList(),
  ...
  Widget _buildImage(T image, int index) {
    return GestureDetector(
      ...
      child: ClipRRect(
        ...
        child: CachedNetworkImage(
          ...
          fit: BoxFit.cover,
          placeholder: (context, url) => ShimmerContainerWidget(height: 180.h),
