...
import 'package:raw_chem/common/widgets/bouncing_ball_refresh_indicator.dart';
...
class _PullToRefreshState extends State<PullToRefresh> {
  ...
  void _scrollListener() {
    if (widget.enableLoadMore &&
        ...
        _scrollController.position.pixels >=
            _scrollController.position.maxScrollExtent - widget.loadMoreTriggerOffset) {
      _handleLoadMore();
    }
  }
  ...
  Widget build(BuildContext context) {
    final refreshProgress = (_dragOffset / widget.refreshTriggerDistance).clamp(0.0, 1.0);
    ...
    return NotificationListener<ScrollNotification>(
      onNotification: (notification) {
        ...
        if (notification is ScrollEndNotification) {
          if (widget.enableRefresh && _dragOffset >= widget.refreshTriggerDistance) {
