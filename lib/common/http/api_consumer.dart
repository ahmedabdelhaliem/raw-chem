  ...
  Future<Either<Failure, Map<String, dynamic>>> get(
    ...
    apiCall() async {
      try {
        ...
        return Right<Failure, Map<String, dynamic>>(response.data as Map<String, dynamic>);
  ...
  Failure _handleDioError(DioException error) {
    switch (error.type) {
      ...
      case DioExceptionType.badResponse: //400-500
        if (error.response?.data != null) {
          try {
            ...
            final Map<String, dynamic> decoded = data is String ? json.decode(data) : data;
            ...
            if (error.response?.statusCode == 401) {
              ...
              navigatorKey.currentContext?.push(AppRouters.loginView);

              return UnauthorizedFailure(message: error.response?.data['message'] ?? 'غير مصرح لك');
            }
            if (error.response?.statusCode == 413) {
              scaffoldMessengerKey.currentContext!.showToast(text: 'File size is too large');
            ...
            if (error.response?.statusCode == 410) {
              scaffoldMessengerKey.currentContext!.showToast(text: 'لقد تعرضت للحظر ');
            ...
            if (decoded.containsKey('message')) {
              ...
              if (decoded.containsKey('errors') && decoded['errors'] is Map) {
                ...
                if (errorMessages.isNotEmpty) {
                  scaffoldMessengerKey.currentContext!.showToast(text: errorMessages[0]);
      ...
      case DioExceptionType.connectionError:
        ...
        // Distinguish between no internet and server unreachable
